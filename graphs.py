...
def bar_graph(data, go, st):
    ...
    Bars = list(go.Bar(name=name, x=df["Timestamp"].astype(str), y=df[name]) for name in columns)
...
def pie_graph(data,go,st,make_subplots):
    ...
    Pies = list(go.Pie(labels=columns, values = list(row[col] for col in columns), name=row["Timestamp"] ) for index, row in df.iterrows())
    # print(Pies)
    k=0
